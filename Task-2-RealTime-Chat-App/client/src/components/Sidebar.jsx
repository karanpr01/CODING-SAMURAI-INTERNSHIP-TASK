...
const Sidebar = ({selectedUser, setSelectedUser}) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-shite ${selectedUser ? "max-md:hidden" : ''}`}>
        <div className='pb-5'>
        ...
        <div className='flex flex-col'>
            {userDummyData.map((user, index) => (
                ...
                className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                    ...
                    {index > 2 && <p className='absolute top-4 right-4  text-xs h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50 text-white'>{index}</p>}
                </div>
            ))}
        </div>
    </div>
